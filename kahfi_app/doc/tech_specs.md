...
In Kahfi App, flavors are divided by its development environments. So it can be either development, staging (UAT), or production. Each of these flavor produces different binary variants with different Application IDs (for Android) or Bundle IDs (for iOS).
...
1. `com.kahfiapp.android.dev` for development
2. `com.kahfiapp.android.stg` for staging (UAT)
3. `com.kahfiapp.android` for production

As for iOS, we use Bundle IDs:

1. `com.kahfiapp.ios.dev` for development
2. `com.kahfiapp.ios.stg` for staging (UAT)
3. `com.kahfiapp.ios` for production
...
The following diagram shows how Kahfi App's development environments related with the Firebase projects.
...
**Step 1.** Go to [https://console.firebase.google.com/](https://console.firebase.google.com/) Create a new Firebase project. It's best to name it "Kahfi App-Prod" to indicate that it's production mode and to distingue it from the rest of the projects.
...
flutterfire config  \
      ...
      --ios-bundle-id=com.kahfiapp.ios.dev \
      --android-package-name=com.kahfi_app.mobile.dev \
...
flutterfire config  \
      ...
      --ios-bundle-id=com.kahfiapp.ios.stg \
      --android-package-name=com.kahfi_app.mobile.stg \
...
flutterfire config \
      ...
      --ios-bundle-id=com.kahfiapp.ios \
      --ios-out=ios/config/prod/GoogleService-Info.plist \
      --android-package-name=com.kahfi_app.mobile \
      --out=lib/src/app/firebase/firebase_options_stg.dart
```
...
**Step 0.** Open file `ios/Runner.xcworkspace` to open the Kahfi App project in XCode.
...
**Step 6.** Login to your [AppstoreConnect](https://appstoreconnect.apple.com/). Select the app (in this case "Kahfi App"). Go to "TestFlight" tab. It will take up to 30 minutes after upload success to get the build shown on the build list.
